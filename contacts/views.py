...
def inquiry(request):
    ...
    if request.method=='POST':
        ...
        # to check if user has already contacted regrading same car

        if request.user.is_authenticated:
            user_id=request.user.id
            has_contacted=Contact.objects.all().filter(car_id=car_id,user_id=user_id)
            if has_contacted:
                messages.error(request,"Yoy have already made a inquiry about this car")
        ...
        # send_mail(
        #  ' New Car Inquiry',
        #   'You Have New In Quirty'+ car_title,
        #    '',
        #     [''],
        #        fail_silently=False,
        #      ) 
